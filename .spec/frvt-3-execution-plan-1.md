...
- The four documents in `.spec` are **authoritative**. Everything in `research/` (POC, standards, prototype) is **supporting only** and must not override a spec.
  - [frvt-3-resolver-requirements-1.md](./frvt-3-resolver-requirements-1.md) — requirements and assumption ids (A1–A27).
  - [frvt-3-resolver-1.md](./frvt-3-resolver-1.md) — resolver + ingest algorithms, contracts, test plan.
  - [frvt-3-server-db-api-design-1.md](./frvt-3-server-db-api-design-1.md) — server, database, HTTP API.
  - [frvt-3-ui-design-1.md](./frvt-3-ui-design-1.md) — React frontend.
...
- **Frontend approach (decided):** build `frvt/web/` **fresh** with **npm** (`package-lock.json`). The prototype at `research/prototypes/frontier_web/` is a **visual/porting reference only**; leave it untouched and do **not** import its types (they predate the reconciled contract). Mirror [server §7.2](./frvt-3-server-db-api-design-1.md) instead.
...
Matches [server §9](./frvt-3-server-db-api-design-1.md) and [UI §10](./frvt-3-ui-design-1.md). Split any module that approaches the size limits rather than letting it grow.
...
- Create the `frvt/` tree above plus `requirements.txt`, `requirements-dev.txt`, `pyproject.toml` (ruff/mypy/black config), `docker-compose.yml` (Postgres 15, container `5432` → host `5433`), `.env.example`, `alembic.ini`, and an empty `migrations/` env. Pin dependency lower bounds per [server §4.1](./frvt-3-server-db-api-design-1.md).
- Implement: settings object (`BaseSettings`, [§5.2](./frvt-3-server-db-api-design-1.md) variables), logging module with `TRACE` ([§5.6](./frvt-3-server-db-api-design-1.md)), HTTP Basic middleware using `secrets.compare_digest` + `WWW-Authenticate` ([§5.3](./frvt-3-server-db-api-design-1.md)), the uniform error envelope + exception handlers + fixed `code` vocabulary ([§5.4](./frvt-3-server-db-api-design-1.md)), DB engine/session dependency (`get_session`, commit on success / rollback on error), `GET /api/health` with a DB ping (`200` ok / `503` when down, [§7.10](./frvt-3-server-db-api-design-1.md)), and a static-file mount placeholder after the API routes ([§5.5](./frvt-3-server-db-api-design-1.md)).
...
- Implement `resolver/types.py` (`VerseId`, `RefRange`) and `resolver/parse_ref.py` helpers `parse_ref`, `expand`, `format_bcv`, `covers`, `index_in_range` per [resolver §4](./frvt-3-resolver-1.md). Grammar is `bcv` (`^[A-Z1-6]{3} [0-9]+:[0-9]+$`) and `bcvRange` (`...(-[0-9]+)?$`); verse `0` is valid; **parts are never in the string** and cross-chapter ranges are invalid — both raise `ReferenceError`.
...
- SQLAlchemy 2.0 typed declarative models for `translation`, `verse_span`, `versification_scheme`, `translation_versification`, `mapping_record`, and the `relation_type` enum per [server §6](./frvt-3-server-db-api-design-1.md). Include: unique `(translation_id, book, chapter, verse, part)` (null vs non-null `part` are distinct rows), unique `(translation_id, seq)`, the **partial** unique index on `translation_id where preferred`, FK delete rules (`RESTRICT` on `versification_scheme.based_on_id` and `translation_versification.scheme_id`; `CASCADE` on spans and associations), unique case-insensitive `translation.name`, and indexes `(scheme_id, source_ref)`, `(scheme_id, relation)`, `(translation_id, book, chapter)`.
...
**Goal:** the ingest module callables the API will later persist ([server §8.2](./frvt-3-server-db-api-design-1.md); [resolver §7–§8](./frvt-3-resolver-1.md)). No DB access, no commits.

- `ingest/types.py`: `ParsedSpan`, `ParsedScheme`, `IngestIssue` (`kind` ∈ `missing|invalid`), `ProjectIngestResult`, `MappingRecordDTO`.
- `derive_mappings.py`: `derive_mapping_records` per [resolver §7.1](./frvt-3-resolver-1.md), including `classify_mapped` (shift vs renumber, §7.2) and `partial` rows with the part in the dedicated column (never in the ref). `basedOn`/`maxVerses`/`verification` produce no rows.
- `usx_parse.py`: USX 3.0 → `ParsedSpan` sequence per [resolver §8.4](./frvt-3-resolver-1.md) (milestone verse pairs, document-order `seq`, drop `<note>` inner text; interim comma-verse rule: full text on first number, empty on the rest).
- `vrs_convert.py`: VRS → ingredient per [§8.3](./frvt-3-resolver-1.md) (`maxVerses` book lines, `=` mapping lines, `#` comments ignored, `basedOn` defaults to `org`); unsupported constructs → `IngestIssue(kind="invalid")`.
...
- `ingest_api.py`: `ingest_project(archive_bytes)` and `ingest_versification(file_bytes, filename)` orchestration ([§8.1–§8.2](./frvt-3-resolver-1.md)). `usfm_convert.py` is a clear fail-closed stub (out of scope; samples are USX).

**Acceptance** (using real assets, [resolver §10](./frvt-3-resolver-1.md) T7–T9, T13): derive rows from `eng.json`/`validated.json` (`PSA 3:0-8` classified `shift`; partial part lands in the `part` column; exclude/merge counts correct); VRS convert of a sample `versification.vrs` (`GEN` chapter 1 max = 31; `GEN 31:55` mapped); USX parse of a short book (e.g. `PHM.usx`) yields monotonic `seq` with non-empty verse content; a zip missing the USX tree or the `.vrs` yields `IngestIssue(kind="missing")`; an unparseable ingredient yields `kind="invalid"`.

### Phase 4 — Bootstrap and canonical seeding

**Goal:** canonical numbering-space anchors and schemes so `basedOn` lookups resolve and the resolver can walk chains on a clean install ([server §5.7](./frvt-3-server-db-api-design-1.md), [reqs A21–A22](./frvt-3-resolver-requirements-1.md)).
...
**Goal:** the in-process resolver ([resolver §6](./frvt-3-resolver-1.md); contract [§3.1 / server §8.1](./frvt-3-server-db-api-design-1.md)). Coordinate-only — it must never read `verse_span.content`.
...
**Acceptance** (fixtures against seeded `eng`/`org`, [resolver §10](./frvt-3-resolver-1.md) T1–T6, T10–T13): identity `JHN 3:16`; `shift` `PSA 3:1` → `PSA 3:2`; `GEN 31:55` → `GEN 32:1` (shift/renumber); `exclude` → empty `target_spans`; `merge` returns siblings; a `bcvRange` input does not raise and yields single-verse spans; missing ancestor raises `LookupError`; a `complex` case returns a hull with `edges` each carrying a relation; a partial resolves via the separate `part` while `parse_ref("SIR 36:13a")` raises.

### Phase 6 — API foundation and CRUD

**Goal:** the read/write CRUD surface and the shared response contracts ([server §7](./frvt-3-server-db-api-design-1.md)).

- Pydantic models per [§7.2](./frvt-3-server-db-api-design-1.md) (including `RelationType`, `ResolvedSpan` with structured coords, `ResolveEdge`, `NavRef`, delta/misalignment DTOs). Pagination envelope `{items, total}` with `limit`/`offset` per [§7.1](./frvt-3-server-db-api-design-1.md).
- Routers: translations ([§7.3](./frvt-3-server-db-api-design-1.md)), scripture spans ([§7.4](./frvt-3-server-db-api-design-1.md), ordered by `seq`), versifications ([§7.5](./frvt-3-server-db-api-design-1.md), detail returns `ingredient`), associations ([§7.6](./frvt-3-server-db-api-design-1.md)) enforcing the preferred invariants (at most one preferred; make-preferred clears the prior in one transaction; the preferred association cannot be deleted). Map errors to the envelope/status table ([§5.4](./frvt-3-server-db-api-design-1.md)).

**Acceptance** (TestClient, [§10.3](./frvt-3-server-db-api-design-1.md)): translation CRUD incl. duplicate case-insensitive name → `409` and deleting a translation still referenced as `based_on_id` → `409`; association invariants (second preferred clears the first; making an unassociated scheme preferred → `409`; deleting the preferred → `409`, non-preferred → `204`); seeded anchors are excluded from `GET /api/translations`; error-contract cases `404` (missing id) and `413` (over `MAX_UPLOAD_BYTES`).

### Phase 7 — Ingest and resolve endpoints

**Goal:** wire the two ports and the coordinate-interpreting endpoints ([server §7.7–§7.9, §8](./frvt-3-server-db-api-design-1.md)).

- `ingest_port`: persist `ingest_project` / `ingest_versification` results in **one transaction** (all-or-nothing), look up `basedOn` → `based_on_id` by case-insensitive name, call `derive_mapping_records` on insert, and for a project create the `preferred` association ([§8.2](./frvt-3-resolver-1.md) responsibilities). `source_format` is inferred from the zip (USX present → `usx`; USFM-only → `usfm`; both → `usx`).
- `resolver_port`: owns per-request scheme selection (the `*_versification` override validated as associated, else the preferred scheme), fills each `ResolvedSpan`'s structured `book`/`chapter`/`verse` by parsing the emitted single-verse ref, attaches `verse_span.seq`/content when a stored span exists, passes `edges` through, and maps `ReferenceError` → `400` while any residual `LookupError` → `422` ([§8.1](./frvt-3-server-db-api-design-1.md)).
- Routers: `POST /api/ingest/project` and `POST /api/versifications/upload` ([§7.7](./frvt-3-server-db-api-design-1.md), `missing` → `400`, `invalid` → `422`, `413` over size cap); `GET /api/resolve` with the `404`/`409`/`400`/`422` pre-check precedence ([§7.8](./frvt-3-server-db-api-design-1.md)); `GET /api/translations/{id}/navigation`, `GET /api/resolve/deltas`, `GET /api/resolve/misalignments` with server-owned categorization (heuristics + known-divergence table) and discrete `navigation_ref`/`navigation` derivation ([§7.9](./frvt-3-server-db-api-design-1.md)).
...
**Goal:** the SPA shell, API client, and manage screens ([UI §5–§7, §9–§10](./frvt-3-ui-design-1.md)).

- Scaffold `frvt/web/` with npm and Vite (`outDir = dist`). Build: `api/` client (`apiGet`/`apiSend`/`apiUpload` with `credentials: "same-origin"`), `ApiError` mapping the [§5.4](./frvt-3-server-db-api-design-1.md) envelope, and `types.ts` mirroring [server §7.2](./frvt-3-server-db-api-design-1.md) (structured `ResolvedSpan`, `edges`, `NavRef`, `navigation`). Router/`AppShell`; `/manage/translations` and `/manage/versifications` pages; modals for upload project/versification, rename, associate, remove association, and confirm delete ([§6.2](./frvt-3-ui-design-1.md)); the empty-state upload flow ([§6.3](./frvt-3-ui-design-1.md)). FastAPI serves `frvt/web/dist/`.
...
**Goal:** the two-column aligned viewer and SVG overlay ([UI §6–§8](./frvt-3-ui-design-1.md)).

- `ViewerSession`: URL-param source of truth ([§7.1](./frvt-3-ui-design-1.md)), caches keyed to include the selected versification ([§7.2](./frvt-3-ui-design-1.md)), the resolve cycle ([§7.6](./frvt-3-ui-design-1.md)), and the cross-chapter follower load-before-scroll ([§6.4](./frvt-3-ui-design-1.md)). Build `ScriptureColumn`/`VerseList`/`VerseSpan` (with `data-seq`/`data-ref`/`data-part`), `ColumnChrome` (BCV selectors + per-request scheme select that never changes the preferred, [§6.5](./frvt-3-ui-design-1.md)), `JumpMenu` (navigate only via structured `navigation`; never parse ranges, [§6.6](./frvt-3-ui-design-1.md)), and `MappingOverlay` + `OverlayController` + `visualLanguage` + `drawPlan` covering every relation topology including the `complex` graph (one connector per `edges` entry) and `exclude` connector-to-void ([§8.2, §8.4–§8.5](./frvt-3-ui-design-1.md)). `toResolveArgs` sends the part separately ([§7.4](./frvt-3-ui-design-1.md)); verse 0 displays as `Title (0)`.
...
- The specs' own interim rules stand until superseded there: the composition table and shift/renumber classifier ([resolver §6.5, §7.2](./frvt-3-resolver-1.md)), exotic multi-character part suffixes, and VRS constructs that cannot round-trip (fail closed).
- Non-viewer scripture query, versification detection/sniffer, mobile/responsive layout, and production authentication/scaling/multi-tenancy ([server §2.2](./frvt-3-server-db-api-design-1.md), [UI §2.2](./frvt-3-ui-design-1.md)).
