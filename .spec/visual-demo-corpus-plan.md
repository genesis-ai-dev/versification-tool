...
canonical_location: temp/visual-demo-corpus-plan.md
todos:
  - id: phase-0-spec
...
**Canonical plan location:** [`temp/visual-demo-corpus-plan.md`](temp/visual-demo-corpus-plan.md) — update this file when iterating on the plan.
...
|---|---|---|
| Ingredient coordinate tuning (Phases 6–7, 11) | Yes — verification/parity tables | No (fixture data only) |
| Resolver interim binding change (product contract) | Yes — case constants | Yes — append **new** subsection `ADD-*-002` |
| New jump heuristic (product contract) | Yes — ingredients + walkthrough | Yes — new subsection (do not append rows to shipped subsections) |
| Pure test/fixture helper work | Fixture spec only | No |
...
[`.spec/visual-demo-corpus-1.md`](.spec/visual-demo-corpus-1.md):

- Design rationale, USX book set, orphan-mapping warning
- Full scheme JSON with real `maxVerses` (or “generated by `demo_max_verses()`”)
- Verification + category tables (§3)
- Seed algorithm (§5)
- Link to [`.test/visual-demo-walkthrough.md`](.test/visual-demo-walkthrough.md) for browser steps
- Maintenance contract: `VERIFICATION_CASES` is canonical; tests enforce it
- Link to this plan at [`temp/visual-demo-corpus-plan.md`](temp/visual-demo-corpus-plan.md)
...
| File | Action |
|---|---|
| [`temp/visual-demo-corpus-plan.md`](temp/visual-demo-corpus-plan.md) | **This plan** — maintain here |
